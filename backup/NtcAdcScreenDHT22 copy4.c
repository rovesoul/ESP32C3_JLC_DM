...
void oled_display_task(void *pvParameter)
{
    ...
    while(1) {
        ...
        // ========== 只清除左侧文字区域 ==========
        OLED_ClearArea(TEXT_START_X, 0, 128, 64);  // 只清除右半屏
        ...
        snprintf(buffer, sizeof(buffer), "H:%.1f%%", dhtHumidity);
        ...
        // 每次清除曲线区域并重绘
        OLED_ClearArea(CURVE_START_X + 1, 1, 62, 62);  // 清除曲线区域（不清除边框）
